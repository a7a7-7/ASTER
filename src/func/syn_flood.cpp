...
int syn_flood(int sock, char dest_ip[], char src_ip[], int dport, int sport)
{
    ...
    if (dest_ip != "")
        ...
        std::cerr << "Destination address cannot be null" << std::endl;
    ...
    {
        std::cerr <<"Error setting IP_HDRINCL. Error number : " << errno << std::endl << "Error message : " << strerror(errno) << std::endl;
        exit(0);
    }
